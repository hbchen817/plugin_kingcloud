...
int topo_join() {
    ...
    const char    *payloads[] = {R"({"id":11,"code":0,"params":[{"productKey":"R204040WJKG","deviceName":"1122334455667788"}]})"};
    for (int i = 0; i < 1; i++) {
        fprintf(stdout, "\n");
        while (map_any_first(params) != NULL) {
            map_any_iterator *iter = map_any_first(params);
...
int main(int argc, char **argv) {
    ...
    if (true) {
        ...
        config_func_t *config  = &g_config.topo_join;
        map_any       *params  = map_any_create();
        const char    *message = R"({"id":1679976069,"code":0,"params":[{"deviceName":"70B3D5058002634D","productKey":"RE03010TouC"}]})";
        common_mqtt_subscribe("/sys/R101Gateway/040D84FFFE3BEDE3/thing/topo/add_reply", message, strlen(message), config->res_topic, config->res_payload,
                              params);
        for (map_any_iterator *iter = map_any_first(params); iter != NULL; iter = map_any_next(iter)) {
            char *str = any_to_json_str(&iter->val0);
            printf("\t%s -> %s\n", iter->key, str);
