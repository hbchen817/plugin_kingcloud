...
//     cJSON_AddStringToObject(root, "clientId", instance.clientId);
//     cJSON_AddStringToObject(root, "vendorCode", instance.vendorCode);
//     char *jsonStr = cJSON_Print(root);
//     log_info("cipher text string: %s\n", jsonStr);

//     // decode hex
//     int dataLen = sizeof(instance.productSecret);
//     char hexStr[2 * dataLen + 1];
//     memset(hexStr, 0, 2 * dataLen + 1);
//     mqtt_decodeHex(hexStr, instance.productSecret, dataLen);
//     free(jsonStr);
//     cJSON_Delete(root);

//     // TODO: aes加密后数值赋给cipherText
//     // AES_Init(hexStr);
//     char cipherText[128];
...
//     cJSON_AddStringToObject(root, "cipherText", cipherText);
//     cJSON_AddStringToObject(root, "productKey", instance.productKey);
//     char *jsonStr = cJSON_Print(root);

//     // 尝试请求金山云
//     log_info("register gateway: %s", jsonStr);
